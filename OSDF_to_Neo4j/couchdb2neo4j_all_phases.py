...
print "Approximate number of documents found in CouchDB file '%s' (likely includes _hist entries which are ignored) = %s" % (sys.argv[1],len(docList))
n = 0
...
for x in docList:
    if re.match(r'\w+\_hist', x['id']) is None: # ignore history documents
        ...
        if 'node_type' in res: # if no node type, need to ignore     
            ...
            o.write("%s\n" % (cstr.encode('utf-8')))
            if n > 0 and n % 50 == 0: # very likely that for each doc, tens of transactions need to occur. Break up commits here.
                o.write(';\n')
                o.write('COMMIT\n')
                o.write('BEGIN\n')
        if n % 1000 == 0:
            print "%s documents processed and respective Cypher statements written to file." % (n)
        n += 1
...
breaks1 = 500 # breakup the Neo4j commit file by rough estimate of edges per document (1 doc ~= 10 edges)
breaks2 = 10000 # break up the terminal output by number of edges added
# Iterate over each doc from CouchDB and insert the nodes into Neo4j.
for x in docList:
    if re.match(r'\w+\_hist', x['id']) is None: # ignore history documents
        singleNode = {} # reinitialize dict at each new document
        ...
        if tot > breaks2: # log to terminal
            print "%s\t\tedges processed." % (breaks2)
            breaks2 += 10000

        breaks1 += 1
        if tot > breaks1: # commit every 500 Cypher statements
            o.write(';\n')
            o.write('COMMIT\n')
            o.write('BEGIN\n')
            breaks1 += 500
...
print "Finished. Added statements to attach a total of %s edges." % (tot)
