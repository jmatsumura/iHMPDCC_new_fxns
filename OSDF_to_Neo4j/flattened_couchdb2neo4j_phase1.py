...
cypher.run("CREATE INDEX ON :Case(node_type)")
cypher.run("CREATE INDEX ON :File(node_type)")
...
for x in docList:
    if re.match(r'\w+\_hist', x['id']) is None: # ignore history documents
        ...
        if 'node_type' in res and res['node_type'] in nodes: # if no node type, need to ignore   
